...
int main()
{
  ...
  */
  ...
  cout << "\nFirst, test port positions through xperim(IDX) and yperim(IDX), with " << bl1.size() << " port(s)\n";
  for(int i=0; i < bl1.size(); ++i)
    cout << "position: " << bl1.xperim(i) << ", " << bl1.yperim(i) << '\n';

  cout << "Next, write block obj. \'bl1\' into file \"test-block-globals.html\"\n";
  ofstream ofs("test-block-globals.html");
