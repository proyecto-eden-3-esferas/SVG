 ...
 * Key characteristics
   (1) A curve is defined through on-line points (points on the line).
       ...
       is control points, which kind of direct the drawing of the line.
...
/* class base_mp_point has no member variables
   ...
   - void set_control_of(F dr, F dist, const pair_t& pt, pair_t& ctrl)
 */
...
public:
  ...
  public:
    ...
    void set_control(F dist, pair_t& ctrl) const {base_t::set_control_of(dir, dist, pt, ctrl);};
    void set_control(F dr, F dist, pair_t& ctrl) const {base_t::set_control_of( dr, dist, pt, ctrl);};
    void set_pre(          F dist) {base_t::set_control_of(dir + std::numbers::pi_v<F>,
                                                           dist, pt, prept);};
    void set_pre(    F dr, F dist) {base_t::set_control_of(dr + std::numbers::pi_v<F>,
                                                           dist, pt, prept);};
    void set_post(         F dist) {base_t::set_control_of(dir, dist, pt, postpt);};
    void set_post(   F dr, F dist) {base_t::set_control_of( dr, dist, pt, postpt);};
    // Constructor(s):
...
public:
  ...
  bool has_dirs;
  /* lastidx(IDX) returns the last valid index into points
   ...
   */
  virtual void set_dir(int pre, int idx, int post);
